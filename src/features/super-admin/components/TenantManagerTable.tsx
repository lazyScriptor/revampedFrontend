...
const TenantManagerTable: React.FC = () => {
    ...
    const handleSaveEdit = () => {
        ...
        updateMutation.mutate({
            tenantId: selectedTenant.tenant_id as number,
            updates: editForm,
        });
        setEditOpen(false);
    };
        ...
        >
            ...
            <Box sx={{ p: 2, borderBottom: '1px solid #1e293b' }}>
                ...
                <Typography variant="caption" sx={{ color: '#64748b' }}>
                    {tenants?.length || 0} registered tenants
                </Typography>
            </Box>
            ...
            <Box
                sx={{
                    display: 'grid',
                    gridTemplateColumns: '2fr 1fr 1fr 1fr 0.5fr 2fr',
                    px: 2,
                    py: 1,
            ...
            {isLoading ? (
                ...
                tenants?.map((tenant: Record<string, unknown>, idx: number) => (
                    <Box
                        key={tenant.tenant_id as number}
                        sx={{
                            display: 'grid',
                            gridTemplateColumns: '2fr 1fr 1fr 1fr 0.5fr 2fr',
                            px: 2,
                    ...
                    >
                        <Typography variant="body2" sx={{ color: '#e2e8f0', fontSize: '0.8rem', fontFamily: 'monospace' }}>
                            {tenant.db_name as string}
                        </Typography>

                        <Box>
                            <Chip
                                label={tenant.subscription_status as string}
                                size="small"
                                sx={{
                                    backgroundColor: `${statusColors[(tenant.subscription_status as string)] || '#64748b'}20`,
                                    color: statusColors[(tenant.subscription_status as string)] || '#64748b',
                                    fontSize: '0.65rem',
                                    height: 22,
                                    fontWeight: 600,
                                }}
                        ...
                        <Box>
                            <Chip
                                label={tenant.tier as string}
                                size="small"
                                variant="outlined"
                                sx={{
                                    borderColor: tierColors[(tenant.tier as string)] || '#64748b',
                                    color: tierColors[(tenant.tier as string)] || '#64748b',
                                    fontSize: '0.65rem',
                                    height: 22,
                                }}
                            />
                        ...
                        <Typography variant="body2" sx={{ color: '#94a3b8', fontSize: '0.8rem' }}>
                            {tenant.max_users as number}
                        </Typography>

                        <Typography variant="body2" sx={{ color: '#94a3b8', fontSize: '0.8rem', fontVariantNumeric: 'tabular-nums' }}>
                            {tenant.userCount as number}
                        </Typography>
                        ...
                        <Box sx={{ display: 'flex', gap: 0.5 }}>
                            ...
                            <Tooltip title="Suspend Tenant">
                                <IconButton
                                    size="small"
                                    onClick={() => {
                                        if (window.confirm(`Suspend "${tenant.db_name}"?`))
                                            suspendMutation.mutate(tenant.tenant_id as number);
                                    }}
            ...
            {/* Edit Dialog */}
            <Dialog open={editOpen} onClose={() => setEditOpen(false)} maxWidth="xs" fullWidth PaperProps={{ sx: { backgroundColor: '#1e293b', color: '#f1f5f9' } }}>
                ...
                <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: '16px !important' }}>
                    ...
                    <TextField
                        ...
                        sx={{ input: { color: '#f1f5f9' }, label: { color: '#94a3b8' }, '.MuiOutlinedInput-notchedOutline': { borderColor: '#334155' } }}
                    />
                </DialogContent>
                <DialogActions>
            ...
            {/* Impersonate Dialog */}
            <Dialog open={impersonateOpen} onClose={() => setImpersonateOpen(false)} maxWidth="xs" fullWidth PaperProps={{ sx: { backgroundColor: '#1e293b', color: '#f1f5f9' } }}>
                ...
                <DialogContent sx={{ pt: '16px !important' }}>
                    <Typography variant="caption" sx={{ color: '#94a3b8', mb: 2, display: 'block' }}>
                        Enter the User ID of the tenant user you want to impersonate. This generates a 15-minute temporary session.
                    </Typography>
                    <TextField
                        size="small"
