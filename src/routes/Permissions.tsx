...
import {
    Box, Tabs, Tab, Paper, Typography, Chip, Button, Avatar,
    IconButton, Tooltip, TextField, InputAdornment, Divider,
...
interface RoleFormDialogProps {
    ...
    onSubmit: (data: { roleName: string; description: string; hierarchyLevel: number }) => void;
    isSubmitting: boolean;
}

function RoleFormDialog({ open, onClose, roleToEdit, onSubmit, isSubmitting }: RoleFormDialogProps) {
    const isEdit = Boolean(roleToEdit);
    const [form, setForm] = useState({ roleName: '', description: '', hierarchyLevel: 10 });
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (!form.roleName.trim()) { setErr('Role name is required.'); return; }
        if (form.hierarchyLevel < 1 || form.hierarchyLevel > 99) {
            setErr('Hierarchy level must be between 1 and 99.'); return;
        }
        onSubmit(form);
    };
    ...
    return (
        <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
            ...
            <DialogContent dividers>
                <Box component="form" id="role-form" onSubmit={handleSubmit}>
                    ...
                    <Stack spacing={2.5} sx={{ pt: 1 }}>
                        <TextField
                            label="Role Name" fullWidth required size="small"
                            value={form.roleName}
                            onChange={(e) => setForm((f) => ({ ...f, roleName: e.target.value }))}
                            disabled={isEdit && Boolean(roleToEdit?.is_system_default)}
                            helperText={isEdit && roleToEdit?.is_system_default ? 'System role names cannot be changed.' : ''}
                        />
                        <TextField
                        ...
                        />
                        <TextField
                            label="Hierarchy Level" type="number" fullWidth required size="small"
                            value={form.hierarchyLevel}
                            onChange={(e) => setForm((f) => ({ ...f, hierarchyLevel: parseInt(e.target.value) || 0 }))}
            ...
            <DialogActions sx={{ px: 3, py: 2 }}>
                ...
                <Button
                    type="submit" form="role-form" variant="contained" disableElevation
...
function AssignUsersDialog({ open, role, allUsers, onClose, onSubmit, isSubmitting }: AssignUsersDialogProps) {
        ...
        <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
            ...
            <DialogContent dividers sx={{ p: 0 }}>
                <Box sx={{ px: 2, pt: 2, pb: 1 }}>
                    <TextField
                        size="small" fullWidth placeholder="Search users…" value={search}
                    ...
                    <Box sx={{ maxHeight: 320, overflow: 'auto' }}>
                        {filteredUsers.map((user) => {
                            ...
                            return (
                                ...
                                >
                                    <Checkbox size="small" checked={checked} readOnly sx={{ p: 0.25 }} />
                                    <Avatar
                                        sx={{
                ...
                <Box sx={{ px: 2, py: 1, borderTop: '1px solid #f1f5f9', bgcolor: '#f8fafc' }}>
                    <Typography variant="caption" sx={{ color: '#64748b', fontSize: '0.65rem' }}>
...
function UserPermissionsTab({ matrixData, isLoading }: { matrixData?: MatrixData; isLoading: boolean }) {
    ...
    const rolePermMap = useMemo(() => {
        ...
        roles.forEach((r) => {
            map[r.role_id] = new Set(r.Permissions?.map((p) => p.permission_id) ?? []);
        });
        return map;
    }, [roles]);
    ...
    const overrideMap = useMemo(() => {
        ...
        overrides.forEach((o) => { map[`${o.user_id}-${o.permission_id}`] = o; });
        return map;
    }, [overrides]);
    ...
    const setOverrideMutation = useMutation({
        mutationFn: async ({ userId, permissionId, grantType }: { userId: number; permissionId: number; grantType: 'grant' | 'revoke' }) => {
            await api.post('/permission-management/user-override', { userId, permissionId, grantType });
        },
    ...
    const removeOverrideMutation = useMutation({
        mutationFn: async ({ userId, permissionId }: { userId: number; permissionId: number }) => {
            await api.delete('/permission-management/user-override', { data: { userId, permissionId } });
        },
        onSuccess: () => qc.invalidateQueries({ queryKey: ['permission-matrix'] }),
        onError: (err: Error) => showError(err.message),
    });
    ...
    const handleOverrideCycle = (userId: number, permId: number) => {
        ...
        const fromRole = user?.Roles?.some((r) => rolePermMap[r.role_id]?.has(permId)) ?? false;

        if (!existing) {
            setOverrideMutation.mutate({ userId, permissionId: permId, grantType: fromRole ? 'revoke' : 'grant' });
        } else if (existing.grant_type === 'grant') {
            setOverrideMutation.mutate({ userId, permissionId: permId, grantType: 'revoke' });
        } else {
    ...
    const handleRemoveRole = (roleId: number) => {
        ...
        const newIds = selectedUser.Roles.filter((r) => r.role_id !== roleId).map((r) => r.role_id);
        assignRolesMutation.mutate({ userId: selectedUser.user_id, roleIds: newIds });
    };

    const handleAddRole = () => {
        if (!selectedUser || !roleToAdd) return;
        const existing = new Set(selectedUser.Roles.map((r) => r.role_id));
        if (existing.has(roleToAdd as number)) { showError('User already has this role.'); return; }
                ...
                <Box sx={{ overflow: 'auto', flex: 1 }}>
                            ...
                            >
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                    ...
                                    <Box sx={{ minWidth: 0 }}>
                                        <Typography
                                            variant="body2"
                                            sx={{ fontWeight: 600, fontSize: '0.76rem', lineHeight: 1.2, color: '#0f172a' }}
                                            noWrap
                                        >
                                            {user.first_name} {user.last_name}
                                        </Typography>
                                        <Typography variant="caption" sx={{ color: '#64748b', fontSize: '0.62rem' }} noWrap>
                                            @{user.username}
                                        </Typography>
                                    </Box>
                                </Box>
                                {user.Roles?.length > 0 && (
                                    <Box sx={{ mt: 0.5, display: 'flex', flexWrap: 'wrap', gap: 0.4 }}>
                                        {user.Roles.slice(0, 2).map((r) => (
                                            <Chip
                                                key={r.role_id} label={r.role_name} size="small"
                                                sx={{ height: 14, fontSize: '0.52rem', '& .MuiChip-label': { px: 0.6 } }}
                                            />
                                        ))}
                                        {user.Roles.length > 2 && (
                                            <Chip
                                                label={`+${user.Roles.length - 2}`} size="small"
                                                sx={{ height: 14, fontSize: '0.52rem', bgcolor: '#f1f5f9', '& .MuiChip-label': { px: 0.6 } }}
                                            />
                                        )}
                                    </Box>
                                )}
                    ...
                    {filteredUsers.length === 0 && (
                        <Box sx={{ p: 3, textAlign: 'center' }}>
                            <Typography variant="caption" sx={{ color: '#94a3b8' }}>No users found</Typography>
                        </Box>
                    )}
                </Box>
            </Paper>
                ...
                <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 2, minWidth: 0 }}>
                    ...
                    <Paper elevation={0} sx={{ p: 2, border: '1px solid #e2e8f0', borderRadius: 2 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                            <Avatar sx={{ width: 42, height: 42, bgcolor: '#3b82f6', fontWeight: 700, fontSize: '0.9rem' }}>
                            ...
                            <Box>
                                ...
                                <Typography variant="caption" sx={{ color: '#64748b' }}>@{selectedUser.username}</Typography>
                            </Box>
                        </Box>
                    </Paper>
                    ...
                    <Paper elevation={0} sx={{ p: 2, border: '1px solid #e2e8f0', borderRadius: 2 }}>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1.5 }}>
                            ...
                            <Button
                                ...
                                variant="outlined" onClick={() => setAddRoleOpen(true)}
                                sx={{ fontSize: '0.7rem', py: 0.3, px: 1, textTransform: 'none', minWidth: 0 }}
                            >
                                Add Role
                            </Button>
                        </Box>
                        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, minHeight: 28 }}>
                            ...
                            {selectedUser.Roles?.map((role) => {
                                ...
                                return (
                                    <Chip
                                        ...
                                        size="small"
                                        onDelete={rd?.is_system_default ? undefined : () => handleRemoveRole(role.role_id)}
                                        deleteIcon={<CloseIcon sx={{ fontSize: '11px !important' }} />}
                                        icon={rd?.is_system_default ? <LockOutlinedIcon sx={{ fontSize: '11px !important' }} /> : undefined}
                                        sx={{
                                            fontWeight: 600, fontSize: '0.72rem',
                                            bgcolor: '#eff6ff', color: '#1d4ed8',
                                            '& .MuiChip-deleteIcon': { color: '#93c5fd', '&:hover': { color: '#ef4444' } },
                                        }}
                                    />
                                );
                            })}
                    ...
                    <Paper elevation={0} sx={{ border: '1px solid #e2e8f0', borderRadius: 2, overflow: 'hidden', flex: 1 }}>
                        {/* Header/Legend */}
                        <Box
                            sx={{
                                px: 2, py: 1,
                        ...
                        >
                            ...
                            {[
                                ...
                                <Box key={label} sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                    ...
                                    <Typography variant="caption" sx={{ color: '#64748b', fontSize: '0.62rem' }}>{label}</Typography>
                                </Box>
                            ))}
                            <Typography variant="caption" sx={{ color: '#94a3b8', fontSize: '0.6rem', ml: 'auto' }}>
                                Click a row to cycle: Inherited → Override → Flip → Clear
                            </Typography>
                        </Box>

                        {/* Permission rows by module */}
                        <Box sx={{ overflow: 'auto', maxHeight: 'calc(100vh - 470px)', minHeight: 200 }}>
                            {Object.entries(groupedPermissions).map(([module, perms]) => (
                                <React.Fragment key={module}>
                                    ...
                                    >
                                        <Typography variant="caption" sx={{ fontWeight: 700, fontSize: '0.65rem', color: '#475569', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                                            {module}
                                        </Typography>
                                        <Badge
                                            badgeContent={perms.length} color="primary"
                                    ...
                                    {perms.map((perm) => {
                                        ...
                                        const fromRole = selectedUser.Roles?.some((r) => rolePermMap[r.role_id]?.has(perm.permission_id)) ?? false;
                                        ...
                                        const roleSource =
                                            !override && fromRole
                                                ? selectedUser.Roles?.filter((r) => rolePermMap[r.role_id]?.has(perm.permission_id))
                                                      .map((r) => r.role_name)
                                                      .join(', ')
                                                : '';
                                        ...
                                        return (
                                            <Box
                                                ...
                                                onClick={() => handleOverrideCycle(selectedUser.user_id, perm.permission_id)}
                                                sx={{
                                                    px: 2, py: 0.625,
                                            ...
                                            >
                                                <Box>
                                                    <Typography
                                                        variant="body2"
                                                        sx={{ fontSize: '0.7rem', color: '#334155', fontFamily: 'monospace' }}
                                                    >
                                                ...
                                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexShrink: 0, ml: 1 }}>
                                                    {roleSource && (
                                                        <Typography variant="caption" sx={{ fontSize: '0.58rem', color: '#94a3b8', fontStyle: 'italic' }}>
                                                            via {roleSource}
                                                        </Typography>
                                                    )}
                                                    <Chip
                    ...
                    {/* Add Role Dialog */}
                    <Dialog open={addRoleOpen} onClose={() => { setAddRoleOpen(false); setRoleToAdd(''); }} maxWidth="xs" fullWidth>
                        <DialogTitle sx={{ fontWeight: 700, fontSize: '0.95rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', py: 1.5 }}>
                            Add Role to {selectedUser.first_name}
                            <IconButton size="small" onClick={() => { setAddRoleOpen(false); setRoleToAdd(''); }}>
                                <CloseIcon />
                            </IconButton>
                        </DialogTitle>
                        <DialogContent dividers>
                            <FormControl fullWidth size="small">
                                <Select
                                    value={roleToAdd} displayEmpty
                                    onChange={(e) => setRoleToAdd(e.target.value as number)}
                                    renderValue={(v) =>
                                        v ? availableRolesToAdd.find((r) => r.role_id === v)?.role_name :
                                ...
                                >
                                    {availableRolesToAdd.map((r) => (
                                        <MenuItem key={r.role_id} value={r.role_id}>
                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                                {r.role_name}
                                                <Chip
                                                    label={`L${r.hierarchy_level}`} size="small"
                                                    sx={{
                                    ...
                                    {availableRolesToAdd.length === 0 && (
                                        <MenuItem disabled>User already has all available roles</MenuItem>
                                    )}
                                </Select>
                            </FormControl>
                        </DialogContent>
                        <DialogActions sx={{ p: 2 }}>
                            <Button onClick={() => { setAddRoleOpen(false); setRoleToAdd(''); }} color="inherit">Cancel</Button>
                            <Button
                                onClick={handleAddRole} variant="contained" disableElevation
...
function RoleManagementTab({ allUsers }: { allUsers: UserData[] }) {
    ...
    const assignUsersMutation = useMutation({
        mutationFn: ({ id, userIds }: { id: number; userIds: number[] }) => roleApi.assignUsers(id, userIds),
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ['roles'] });
            qc.invalidateQueries({ queryKey: ['permission-matrix'] });
    ...
    const cloneMutation = useMutation({
        mutationFn: async ({ sourceRoleId, targetRoleId }: { sourceRoleId: number; targetRoleId: number }) => {
            await api.post('/permission-management/clone-role', { sourceRoleId, targetRoleId });
        },
        onSuccess: (_data, vars) => {
            qc.invalidateQueries({ queryKey: ['roles'] });
            qc.invalidateQueries({ queryKey: ['permission-matrix'] });
            const from = roles.find((r) => r.role_id === vars.sourceRoleId)?.role_name;
    ...
    return (
        <Box>
            ...
            >
                ...
                >
                    ...
                    <FormControl size="small" sx={{ minWidth: 130 }}>
                        <Select
                            value={cloneFrom} displayEmpty
                            onChange={(e) => setCloneFrom(e.target.value as number)}
                            renderValue={(v) =>
                                v ? roles.find((r) => r.role_id === v)?.role_name :
                                    <em style={{ color: '#94a3b8', fontSize: '0.72rem' }}>From role</em>
                            }
                            sx={{ fontSize: '0.72rem' }}
                        >
                            {roles.map((r) => <MenuItem key={r.role_id} value={r.role_id} sx={{ fontSize: '0.75rem' }}>{r.role_name}</MenuItem>)}
                        </Select>
                    </FormControl>
                    <Typography variant="caption" sx={{ color: '#94a3b8', fontWeight: 700 }}>→</Typography>
                    <FormControl size="small" sx={{ minWidth: 130 }}>
                        <Select
                            value={cloneTo} displayEmpty
                            onChange={(e) => setCloneTo(e.target.value as number)}
                            renderValue={(v) =>
                                v ? roles.find((r) => r.role_id === v)?.role_name :
                                    <em style={{ color: '#94a3b8', fontSize: '0.72rem' }}>To role</em>
                            }
                            sx={{ fontSize: '0.72rem' }}
                        >
                            {roles.filter((r) => !r.is_system_default && r.role_id !== cloneFrom).map((r) => (
                                <MenuItem key={r.role_id} value={r.role_id} sx={{ fontSize: '0.75rem' }}>{r.role_name}</MenuItem>
                            ))}
                        </Select>
                    </FormControl>
                    <Button
                        ...
                        onClick={() => cloneMutation.mutate({ sourceRoleId: cloneFrom as number, targetRoleId: cloneTo as number })}
                ...
                <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'center' }}>
                    ...
                    <Button
                        ...
                        onClick={() => { setRoleToEdit(null); setRoleFormOpen(true); }}
                                ...
                                <Paper
                                    ...
                                    sx={{
                                        ...
                                        '&:hover': role.is_active
                                            ? { borderColor: `${hColor}50`, boxShadow: `0 2px 14px ${hColor}18` }
                                            : {},
                                    }}
                                >
                                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 1.5 }}>
                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.25 }}>
                                            ...
                                            >
                                                {role.is_system_default
                                                    ? <LockOutlinedIcon sx={{ fontSize: 17, color: hColor }} />
                                                    : <ShieldOutlinedIcon sx={{ fontSize: 17, color: hColor }} />
                                                }
                                            </Box>
                                            <Box>
                                                <Typography variant="body2" sx={{ fontWeight: 700, fontSize: '0.88rem', color: '#0f172a', lineHeight: 1.2 }}>
                                                    {role.role_name}
                                                </Typography>
                                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mt: 0.3 }}>
                                                    <Chip
                                                        label={`Level ${role.hierarchy_level}`} size="small"
                                                        sx={{
                                                            height: 15, fontSize: '0.56rem', fontWeight: 700,
                                                            color: hColor, bgcolor: `${hColor}15`,
                                                            '& .MuiChip-label': { px: 0.7 },
                                                        }}
                                                    />
                                                    {role.is_system_default && (
                                                        <Chip
                                                            label="System" size="small"
                                                            sx={{ height: 15, fontSize: '0.56rem', color: '#92400e', bgcolor: '#fef3c7', '& .MuiChip-label': { px: 0.7 } }}
                                                        />
                                                    )}
                                                    {!role.is_active && (
                                                        <Chip
                                                            label="Inactive" size="small"
                                                            sx={{ height: 15, fontSize: '0.56rem', '& .MuiChip-label': { px: 0.7 } }}
                                                        />
                                                    )}
                                                </Box>
                                            </Box>
                                        ...
                                        <Box sx={{ display: 'flex', gap: 0.25, flexShrink: 0 }}>
                                            <Tooltip title="Assign Users to Role">
                                                <IconButton size="small" onClick={() => setAssignUsersRole(role)}
                                                    sx={{ '&:hover': { bgcolor: '#eff6ff' } }}>
                                                    <GroupAddIcon sx={{ fontSize: 15, color: '#3b82f6' }} />
                                                </IconButton>
                                            </Tooltip>
                                            {!role.is_system_default && role.is_active && (
                                                <>
                                                    <Tooltip title="Edit Role">
                                                        <IconButton size="small" onClick={() => { setRoleToEdit(role); setRoleFormOpen(true); }}
                                                            sx={{ '&:hover': { bgcolor: '#f0fdf4' } }}>
                                                            <EditOutlinedIcon sx={{ fontSize: 15, color: '#10b981' }} />
                                                        </IconButton>
                                                    </Tooltip>
                                                    <Tooltip title="Deactivate Role">
                                                        <IconButton size="small" onClick={() => handleDelete(role)}
                                                            sx={{ '&:hover': { bgcolor: '#fff5f5' } }}>
                                                            <DeleteOutlinedIcon sx={{ fontSize: 15, color: '#ef4444' }} />
                                                        </IconButton>
                                                    </Tooltip>
                                                </>
                                            )}
                                    ...
                                    {role.description && (
                                        <Typography
                                            variant="caption"
                                            sx={{ color: '#64748b', fontSize: '0.68rem', display: 'block', mb: 1, lineHeight: 1.4 }}
                                        >
                                            {role.description}
                                        </Typography>
                                    )}
                                    ...
                                    <Box sx={{ display: 'flex', gap: 1 }}>
                                        <Box sx={{ flex: 1, textAlign: 'center', py: 0.75, bgcolor: '#f8fafc', borderRadius: 1.5 }}>
                                            <Typography variant="caption" sx={{ fontWeight: 800, fontSize: '1.1rem', color: hColor, display: 'block', lineHeight: 1 }}>
            ...
            <RoleFormDialog
                open={roleFormOpen}
                onClose={() => { setRoleFormOpen(false); setRoleToEdit(null); }}
                ...
                onSubmit={(data) => {
                    if (roleToEdit) {
                        updateMutation.mutate({
                            ...
                            data: { role_name: data.roleName, description: data.description, hierarchy_level: data.hierarchyLevel },
                        });
                    } else {
            ...
            {assignUsersRole && (
                <AssignUsersDialog
                    ...
                    onSubmit={(userIds) => assignUsersMutation.mutate({ id: assignUsersRole.role_id, userIds })}
        ...
        <Box sx={{ p: { xs: 2, md: 3 }, maxWidth: 1600, mx: 'auto' }}>
            ...
            <Box sx={{ mb: 3 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
                    <AdminPanelSettingsOutlinedIcon sx={{ color: '#3b82f6', fontSize: 24 }} />
                    <Typography variant="h5" sx={{ fontWeight: 800, color: '#0f172a' }}>
                        Access Control
                    </Typography>
                </Box>
                <Typography variant="body2" sx={{ color: '#64748b' }}>
                    Manage role-based permissions, user-level overrides, and role hierarchies across the platform.
                </Typography>
            </Box>
            ...
            <Paper elevation={0} sx={{ border: '1px solid #e2e8f0', borderRadius: 2 }}>
                <Tabs
                    ...
                    sx={{
                        borderBottom: '1px solid #e2e8f0',
                        px: 1,
                        '& .MuiTab-root': { textTransform: 'none', fontSize: '0.85rem', fontWeight: 500, minHeight: 46 },
